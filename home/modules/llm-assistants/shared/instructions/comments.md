...
- **A nushell comment directly above a `def`.** Nushell renders it as the command's `--help` text, so deleting it blanks the CLI documentation.
- **A section banner**, which names code instead of describing it. A banner is a bare label of at most four words with no verb and no sentence (`# Module options`, `# Formatter configuration`), whether or not a rule of repeated `=`, `-`, or `*` characters wraps it. The exemption covers that label line alone. Every sentence in a file header has to earn its place as an ordinary comment, and a comment describing an ordering is never a banner.
