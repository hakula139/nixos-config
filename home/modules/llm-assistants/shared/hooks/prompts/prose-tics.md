...
3. **Antithesis by negated alternative**: defining something by what it is not. The connector is incidental, so flag the two-clause form ("This isn't about speed. It's about correctness."), the compact `, not` form ("a genuine exemption, not leniency"), and the paraphrases `rather than`, `instead of`, `as opposed to`, and `X over Y`. The Chinese "不是……而是……" counts too. Pass it only when a reader would plausibly choose the thing being negated: "falls back to tabs rather than to no rule" earns it, since "no rule" is what a reader assumes, and so does "return the value, not the pointer". "measure the script rather than assuming" does not, since nobody advocates assuming. Report the fix as deleting the clause and keeping the assertion.
