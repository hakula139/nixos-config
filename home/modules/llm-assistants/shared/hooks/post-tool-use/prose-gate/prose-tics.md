...
5. **Hard-wrapped Markdown prose**: in `.md` content, body paragraphs broken mid-sentence with newlines to cap line width. The convention is one sentence or paragraph per line (soft wrap), so the editor reflows it.
   ...
   - **A comment in a source file.** Comments are hard-wrapped to the file's column limit by convention, so their line breaks are never this tic. Tic 7 is the only wrapping fault a comment can commit.
...
8. **Empty summary**: a sentence opening with "In summary", "Overall", "To recap", "To sum up", or "All in all". A section that concludes needs no recap, so report the fix as deleting the opener or the whole sentence.
9. **Connector pile-up**: "however", "therefore", "moreover", "furthermore", or "additionally" used more than once each in the same passage, or two of them stacked in one sentence. One of each is plenty; repeated, they paper over a missing argument. Quote both occurrences.
10. **Intensifier**: "extremely", "incredibly", "absolutely", "vastly", "hugely", "massively", or "utterly" modifying a claim. They weaken the noun they modify, so report the fix as deleting the adverb.
11. **Absolutist claim about correctness**: "bug-free", "production-ready", "fully verified", "guaranteed", "bulletproof", "rock-solid", or "battle-tested" asserted of code or a result. These claim a completeness no test run establishes. Report the fix as stating what was checked and by what means.
...
- **Ground every flag in a verbatim quote, before any other consideration.** Locate the offending span in the text and copy it character for character. For the character-based tics (em-dash, semicolon, typographic substitutes), confirm the copied span literally contains `—`, `--`, `;`, `…`, or a curly quote. A tilde `~`, a single hyphen `-`, an en dash `–`, and a minus sign `−` are none of those, so a span whose only dash-like character is one of them carries no em-dash and must not be reported as one. Re-read the copied span character by character before you name a character tic, since flagging an em-dash in a line that holds none is an observed failure of this gate. For the word-based tics (empty summary, connector pile-up, intensifier, absolutist claim), confirm the copied span literally contains the offending word. For mechanical parallelism, confirm the copied span contains the actual repeated phrases. For an orphaned last word, confirm the copied span's final line holds one short word, since a final line carrying a whole clause is not an orphan. For a comment or docstring tic, copy the comment as written. If you cannot produce such a quote, there is no violation to report, so return `ok: true`. Never reconstruct punctuation from memory, paraphrase the text into a violation, or quote a phrase the text does not contain. This check outranks the calibration below: an ungrounded flag is a fabrication that costs the user a verification round every time.
...
Scan before you answer. In a few lines, list what you found: every `—`, `--`, `;`, `…` and curly quote, every summary opener, connector, intensifier and absolutist correctness word, and every comment, docstring and user-facing string. Locating the candidates is what catches a tic, so a verdict reached without listing them is a guess.
