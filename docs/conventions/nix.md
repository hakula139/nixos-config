...
- **Multiline text**: keep prompts and prose in adjacent text files or Nix indented strings with literal line breaks. Do not encode paragraphs with `\n` escapes, which hide their layout in the source. Escaped separators in operations such as `concatStringsSep "\n"` are fine.
