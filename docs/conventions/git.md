...
- **PR labels** follow the change: `fix` → `bug`, `feat` → `enhancement`, `docs` → `documentation`, and `refactor`, `ci`, or `chore` use the corresponding label. Check recent comparable PRs for other types and exceptions.
