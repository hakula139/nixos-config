...
`hakula.llm-assistants.proxy.*` fans out to each assistant (`claude-code`, `codex`, `opencode`), and the ACP module applies it to the Cursor CLI it installs. The URL defaults to `http://127.0.0.1:7897` (local mihomo) and can be overridden via `url` or `secretUrlFile`.
