...
`hakula.llm-assistants.proxy.*` fans out to each assistant (`claude-code`, `codex`, `omp`, `opencode`). The URL defaults to `http://127.0.0.1:7897` (local mihomo), overridable via `url` or `secretUrlFile`.
