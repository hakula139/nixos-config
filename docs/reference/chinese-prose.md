...
Codex uses the same rewriter for interactive questions, selected MCP fields, and Markdown `Add File` patches. It preserves `Update File` chunks because they can contain partial paragraphs or fence interiors whose enclosing Markdown is absent. Its adapter returns an explicit allow decision with the updated input. Comment review examines added source lines, and formatting follows rename destinations. OpenCode remains unwired.
